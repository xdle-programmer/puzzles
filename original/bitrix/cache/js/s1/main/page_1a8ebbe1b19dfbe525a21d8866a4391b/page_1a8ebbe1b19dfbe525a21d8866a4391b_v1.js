...
function JCSmartFilter(ajaxURL, viewMode, params, componentId)
{
	...
	{
		this.bindUrlToButton('set_filter', params.SEF_SET_FILTER_URL + "?bxajaxid=" + this.componentId, "comp_" + this.componentId);
		this.sef = true;
	}
	if (params && params.SEF_DEL_FILTER_URL)
	{
		this.bindUrlToButton('del_filter', params.SEF_DEL_FILTER_URL + "?bxajaxid=" + this.componentId, "comp_" + this.componentId);
	}
}
	...
	if (this.form)
	{
		...
		else
		{
			if (this.sef)
			{
				...
				set_filter.setAttribute("disabled", true);
			}
	...
	else if (arItem.VALUES)
	{
		for (var i in arItem.VALUES)
		{
			if (arItem.VALUES.hasOwnProperty(i))
			{
				...
				if (!!control)
				{
					var label = document.querySelector('#'+value.CONTROL_ID);
					if (value.DISABLED)
					{
						if (label)
...
JCSmartFilter.prototype.postHandler = function (result, fromCache)
{
	...
	if (!!result && !!result.ITEMS)
	{
		...
		if (!!modef && !!modef_num)
		{
			...
			else
			{
				...
				else if (result.SEF_SET_FILTER_URL)
				{
	...
	{
		var set_filter = BX('set_filter');
		set_filter.removeAttribute("disabled");
	}
	...
	this.cacheKey = '';
    window.dispatchEvent(new Event("postHandlerFinish"));
};

JCSmartFilter.prototype.bindUrlToButton = function (buttonId, url, componentId)
{
	...
	if (button)
	{
		...
		BX.bind(button, 'click', function (e) {
            if (componentId) {
                url = BX.util.htmlspecialcharsback(url) + "&" + window.location.search.substr(1);
                BX.ajax.insertToNode(url, componentId);
                return BX.PreventDefault(e);
            } else {
